(function() {
  ...
  // DEBUG: Check XLSX global
  setTimeout(() => {
    const dbg = document.createElement('div');
    dbg.style.position = 'fixed';
    dbg.style.top = '10px';
    dbg.style.right = '10px';
    dbg.style.background = '#004E36';
    dbg.style.color = '#fff';
    dbg.style.padding = '8px 16px';
    dbg.style.zIndex = 9999;
    dbg.style.fontWeight = 'bold';
    dbg.textContent = 'ExcelEditFun.js loaded | XLSX: ' +
    ...
    console.log('[DEBUG] window.XLSX:', window.XLSX);
    if (window.XLSX) {
      console.log('[DEBUG] XLSX.read:', typeof window.XLSX.read, window.XLSX.read);
    }
  }, 500);

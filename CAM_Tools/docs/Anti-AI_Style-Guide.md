...
#### Accent Colors — Default (YouTube Blue)
| Token | Hex | Usage |
|-------|-----|-------|
...
#### Accent Colors — Alternate (YouTube Red)

YouTube Red is an approved alternative to the default blue accent. Scripts **should** expose this as a user-facing **settings toggle** (e.g., "Accent: Blue / Red") stored via `GM_getValue`/`GM_setValue`.

| Token | Hex | Usage |
|-------|-----|-------|
| `--accent-primary` | `#ff0000` | Primary actions (YouTube red) |
| `--accent-hover` | `#ff3333` | Hover state for primary |

> **Note:** Only `--accent-primary` and `--accent-hover` change when switching themes. Success, warning, and error colors remain the same regardless of accent choice.
...
:root {
  ...
  /* Accent — default (YouTube Blue) */
  --tm-accent-primary: #3ea6ff;
  --tm-accent-hover: #65b8ff;
  /* To use YouTube Red instead, override the two values above:
     --tm-accent-primary: #ff0000;
     --tm-accent-hover: #ff3333;
  */
...
#### Accent Theme Toggle (JS Helper)

Scripts should let users pick their accent via a settings UI. Store the choice with `GM_getValue`/`GM_setValue` and apply it at startup:

```js
/**
 * Applies the user's chosen accent theme to all --tm-accent-* CSS variables.
...
function applyAccentTheme() {
  ...
  const choice = GM_getValue('tm_accent_theme', 'blue'); // default blue
  const theme = THEMES[choice] || THEMES.blue;

  document.documentElement.style.setProperty('--tm-accent-primary', theme.primary);
  document.documentElement.style.setProperty('--tm-accent-hover', theme.hover);
}
```
...
|---------|------|---------|
| 1.1.0 | 2026-02-15 | Added YouTube Red as alternate accent theme with settings toggle guidance |

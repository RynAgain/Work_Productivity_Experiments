...
(function() {
    ...
    function addDownloadButton() {
        ...
        // 1) Create the download data button
        ...
        // IMPORTANT: "zIndex = '1000 !important'" is invalid. Use a numeric value or setProperty
        ...
        // 3) Append the button and progress tracker to the body
        document.body.appendChild(downloadButton);

        // If progressTracker does not exist, append it:
        ...
        // 4) Add click event to the download data button
        downloadButton.addEventListener('click', function() {
            ...
            .then(storeData => {
                ...
                function fetchItemsForStore(storeId, index) {
                    const headersItems = {
                        'accept': '*/*',
                        'accept-encoding': 'gzip, deflate, br',
                        'accept-language': 'en-US,en;q=0.9',
                    ...
                    .then(data => {
                        ...
                        // If data.itemsAvailability is missing or empty, return an empty array
                        if (!data.itemsAvailability || !Array.isArray(data.itemsAvailability)) {
                            return [];
                        }
                        ...
                        return data.itemsAvailability.map(item => {
                            ...
                            item.reservedQuantity = item.reservedQuantity !== undefined && item.reservedQuantity !== ''
                                ? parseInt(item.reservedQuantity) || 0
                                : 0;
                            item.salesFloorCapacity = item.salesFloorCapacity !== undefined && item.salesFloorCapacity !== ''
                                ? parseInt(item.salesFloorCapacity) || 0
                                : 0;
                            item.wfmoaReservedQuantity = item.wfmoaReservedQuantity !== undefined && item.wfmoaReservedQuantity !== ''
                                ? parseInt(item.wfmoaReservedQuantity) || 0
                                : 0;
                            return item;
                        });
                    })
                ...
                // 6) Fetch items for all stores and compile results
                let completedStores = 0;
                const totalStores = storeIds.length;

                Promise.all(
                    storeIds.map((storeId, index) =>
                        fetchItemsForStore(storeId, index)
                            .then(result => {
                                completedStores++;
                ...
                .then(results => {
                    ...
                    if (allItems.length > 0) {
                        ...
                        progress.innerHTML = 'No items found to download.';
                    }
                })
                .catch(error => {
                    console.error('Error fetching items for all stores:', error);
                    progress.innerHTML = 'Error while fetching items.';
                });
            })
            .catch(error => {
                console.error('Error downloading data:', error);
                progress.innerHTML = 'Error downloading data.';
            });
        });
    }

    // Use MutationObserver to detect changes in the DOM and attempt to add the button
